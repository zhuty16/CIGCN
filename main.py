...
parser.add_argument('--node_dropout_rate', type=float, default=0.3)
...
with tf.Graph().as_default(), tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
    ...
    print('max len: %d, min len: %d, avg len: %.4f' % (np.max(train_dict_len), np.min(train_dict_len), np.mean(train_dict_len)))
